  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-gradient-to-b from-purple-50 to-gray-100'}`}>
      ...
      {[
        ...
        <motion.div
          key={item.id}
          className={`relative flex items-center mb-16 w-full ${index % 2 === 0 ? 'justify-start' : 'justify-end'}`}
        ...
        >
          {/* Year Display */}
          <div className={`w-1/2 ${index % 2 === 0 ? 'text-right pr-8 order-1' : 'text-left pl-8 order-2'}`}>
            <p className="text-gray-400 dark:text-gray-500 text-md font-medium">{item.duration}</p>
          </div>

          {/* Icon in the Center */}
          <div className="w-12 h-12 bg-white dark:bg-gray-800 border-2 border-blue-500 rounded-full flex items-center justify-center shadow-md absolute left-1/2 transform -translate-x-1/2 z-10">
            {item.icon}
          </div>

          {/* Floating Card with Arrow */}
          <div className={`w-1/2 ${index % 2 === 0 ? 'text-left pl-8 pr-8 order-2' : 'text-right pl-8 pr-8 order-1'}`}>
            <div className="relative bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-6 hover:scale-105 transition-transform duration-300 ease-in-out">
              ...
              <div
                className={`absolute top-1/2 transform -translate-y-1/2 w-0 h-0 border-t-8 border-b-8 border-transparent ${
                  index % 2 === 0
                    ? 'border-r-8 border-gray-200 dark:border-gray-700 -left-2'
                    : 'border-l-8 border-gray-200 dark:border-gray-700 -right-2'
                }`}
              ></div>

              {/* Card Content */}
              <h3 className="text-xl font-semibold text-gray-800 dark:text-white">{item.title}</h3>
              <h4 className="text-md text-gray-600 dark:text-gray-300 font-medium">{item.company} — {item.location}</h4>
